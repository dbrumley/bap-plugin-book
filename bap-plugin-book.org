...
[[https://realworldocaml.org/v1/en/html/variants.html][Chapter 6 of RWO]].  Here we are defining a class of variants called
=arm=.  The =match= statement matches every variant in the class, and
is shorthand for:
...
Next, the plugin iterates and prints out the sequence of disassembled
instructions:
#+BEGIN_SRC ocaml
  Seq.iter (Disasm.insns (Project.disasm p)) ~f:(fun (mem,insn) -> 
      Printf.printf "%s %s\n"
...
Let's go over this more slowly.
...
it appears. Each sequence item pair is passed to =f= to do the actual
printing. Note that we use the target-specific printing function
=Target.CPU.addr_of_pc=.

**** TODO Why do we need this target for pc, but not insns themselves?
It seems odd to me that we don't need Target for the insn, but do need
it to print the address. It seems we should have a =Insn.addr= which
returns an address or something.  I'm sure there is a good reason, but
i couldn't explain it here which is why I have this todo.
